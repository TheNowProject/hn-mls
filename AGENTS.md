...
- Read `README.md` for repository orientation and `CONTEXT.md` for canonical MLS terminology before making product, domain, schema, API, or workflow decisions.
- Treat `documents/research/mls-texas/BA-report.md` as the primary analyzed reference and `documents/research/mls-texas/visual-analysis.md`, transcript, and frames as supporting evidence.
...
- Do not use `Property`, `Parcel`, `Listing`, `Transaction`, `Member`, `Agent`, and `Broker` interchangeably. Update `CONTEXT.md` when a term is resolved.
