# Actor scaling recommendation for HouseNow MLS
...
The implemented six perspectives are Agent, Brokerage, Developer, Bank, Regulator and Buyer (`src/data/mockData.js`, `src/App.jsx`, `server/auth.js`). This is current implementation evidence, not the updated target scope.

The repository already models the target Seller authority and the three operating roles:

- `MASTER_PLAN.md` explicitly separates Organization Admin, System Admin and Data Steward from market actors and says none receives blanket business-data access.
- `docs/product/product-requirements-baseline.md` models Owner/Seller ownership claims, representation, consent, Listing oversight and correction/dispute requests.
- `docs/product/phase-6-3-system-admin-build-plan.md` already specifies the Organization Admin and System Admin control planes.

This means the Seller change is a promotion of known domain scope. The implementation delta is a Seller projection/workspace and durable ownership/representation workflows, not a rewrite of Property, Listing, lifecycle, audit or access foundations.
