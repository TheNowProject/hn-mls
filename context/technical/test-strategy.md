...
- Source/channel tests enforce that the dated 357 capture belongs only to the source catalog and that HouseNow remains a Tin bán distribution channel with the supplied icon and status `Chưa phát hành`.
- Reducer tests cover actor, state, and payload guards; independent record lifecycles; automatic PLID/PTID creation; the recoverable VPCC supplement; append-oriented histories; both automatically selected transfer routes; derived work queues; invalid-action atomicity; and versioned persistence/replay.
...
- no landing page, presentation rail, role-handoff prompt, progress percentage, presentation call to action, evidence badge, or repeated environment disclaimer in the product UI.

## End-to-end operational path

1. Open the root URL and verify it resolves directly to the Môi giới work queue with two projected rows, computed metrics, real filters, global search, and no identifiers that have not yet been created.
2. Open each dossier from the table. Verify the linked-object strip keeps NPID, PLID, and PTID separate; permitted tabs expose list/detail data and restricted tabs or fields are omitted for the active role.
3. As Môi giới, select the matching NPID and its complete source set, then submit a valid representation period. Verify missing fields, an invalid date range, incomplete source selection, and a wrong role cannot mutate state.
4. As Người bán, confirm representation and verify that PLID is created automatically with status `Đã khởi tạo`. On Tin bán detail, verify HouseNow appears only in `Kênh phân phối`, uses the supplied icon, and remains `Chưa phát hành`.
5. Record the buyer, confirm readiness, and test finance-sharing both ways. The Ngân hàng queue must omit an unconsented dossier entirely and expose only the permitted projection for a consented dossier. Inspect the Sàn môi giới coordination projection separately.
...
7. Record each valid VPCC signing result. Verify PTID, tax/integration events, and the correct transfer route are created atomically without a manual VMLS action or user-selected route.
8. Complete the Chủ đầu tư intake → transfer confirmation → buyer receipt path for the HĐMB dossier. Independently complete the VPĐKĐĐ result path for the landed-property dossier. Verify both queues, details, statuses, and histories after completion.
9. As Vận hành VMLS, open `Kết nối & nguồn dữ liệu`. Verify 357 appears only in this catalog with owner, URL, category, capture date, coverage, and connection status; open the local screenshot and confirm no dossier claims it as a record-level source.
